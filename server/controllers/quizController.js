...
module.exports = {
  createQuiz: (req, res, next) => {
    ...
    bookshelf.knex('quiz').returning('id').insert({
    ...
    .then((quizID) => {
      ...
      bookshelf.knex('question').returning('id').insert(questions)
      .then((questionIDs) => {
        ...
        bookshelf.knex('answer_option').returning('id').insert(answerOptions)
  ...
  updateQuiz: (req, res, next) =>
    Quiz.where({'quiz_id': req.params.quiz_id})
    .fetch({require: true})
    .then((quiz) =>
      quiz.save(req.body)
  ...
  createAttempt: (req, res, next) => {
    ...
    bookshelf.knex('attempt').returning('id').insert({
      quiz_id: quiz.id,
